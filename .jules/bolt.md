...
## 2024-05-25 - Python Typing Module Overhead
**Learning:** The `typing` module in Python (e.g., `from typing import List, Optional`) introduces an unexpected startup overhead (approx. 40ms) when imported. For ultra-fast CLIs, this import time alone can be a noticeable delay.
**Action:** When working on performance-sensitive Python CLIs, use `from __future__ import annotations` and Python 3.10+ syntax (e.g., `list[str] | None`) to opt-out of the `typing` module startup overhead while preserving type checking and readability.
