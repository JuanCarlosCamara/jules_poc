...
## 2024-05-25 - typing module startup overhead in CLI tools
**Learning:** The standard library `typing` module initialization introduces measurable startup overhead (~2-5ms) which scales linearly with the number of imported generic types, even when heavily cached in modern Python. While small, this delay is notable for highly optimized fast-path CLIs.
**Action:** Use `from __future__ import annotations` and Python 3.10+ native typing syntax (e.g., `list[str] | None`) to preserve readability and full type-checker compatibility while completely eliminating runtime type evaluation and import overhead.
