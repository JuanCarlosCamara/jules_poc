...
**Learning:** Importing the `typing` module adds significant overhead (e.g. ~15-20ms) to the startup time of extremely basic Python CLI scripts.
**Action:** When optimizing ultra-fast CLI tools, use `from __future__ import annotations` and Python 3.10+ native typing syntax (e.g. `list[str] | None`) to avoid the initialization overhead of the `typing` module.
