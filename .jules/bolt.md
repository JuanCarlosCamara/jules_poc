...
**Learning:** Even built-in standard library imports like `from typing import List, Optional` can add measurable startup overhead (e.g., ~15-20ms). For micro-CLIs aiming for instantaneous responses, these imports can double the execution time.
**Action:** Use modern Python 3.10+ typing syntax (like `list[str] | None`) for type annotations instead of importing from `typing`. This allows you to retain static type safety while completely eliminating the module import cost.
