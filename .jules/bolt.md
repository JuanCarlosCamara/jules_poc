...
## 2024-05-14 - Avoiding typing Module Startup Overhead in CLI Tools
**Learning:** Importing the `typing` module (e.g., `from typing import List, Optional`) introduces significant startup overhead (around 80-100ms) which negatively impacts the perceived latency of fast CLI tools.
**Action:** Use `from __future__ import annotations` and modern Python 3.10+ type hint syntax (e.g., `list[str] | None`) to eliminate this overhead while preserving complete type safety and code readability.
